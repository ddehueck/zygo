...
export const fieldGroupStyles = tv({
  extend: focusRing,
  base: "group box-border flex h-9 items-center overflow-hidden rounded-lg border bg-app-bg-surface transition forced-colors:bg-system-field",
...
export function Input(props: InputProps) {
  return (
    <RACInput
      ...
      className={composeTailwindRenderProps(
        props.className,
        "px-3 py-0 min-h-9 flex-1 min-w-0 border-0 outline outline-0 bg-app-bg-surface font-sans text-sm text-app-foreground placeholder:text-app-foreground-muted disabled:text-app-foreground-muted/50 disabled:placeholder:text-app-foreground-muted/50 [-webkit-tap-highlight-color:transparent]",
      )}
    />
  );
}
